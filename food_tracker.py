...
def db() -> psycopg.Connection:
    ...
    # Fail fast instead of hanging (prevents Cloudflare 524)
...
<div class="wrap">
  <div class="topbar">
    <div>
      ...
      <p>Postgres storage + editing + totals + exports.</p>
    </div>
  ...
  <div class="grid">
    <div class="card">
      ...
      <div class="wheelWrap">
        <div class="wheelOuter"><div class="wheelScroll" id="wheelHour"></div><div class="selectBand"></div><div class="fadeTop"></div><div class="fadeBot"></div></div>
        <div class="wheelOuter"><div class="wheelScroll" id="wheelMin"></div><div class="selectBand"></div><div class="fadeTop"></div><div class="fadeBot"></div></div>
        <div class="wheelOuter ampm"><div class="wheelScroll" id="wheelAmPm"></div><div class="selectBand"></div><div class="fadeTop"></div><div class="fadeBot"></div></div>
      </div>
      ...
      <div class="sub" style="margin-top:10px" id="editHint">Tip: Click “Edit” on a log item to load it into the picker.</div>
    </div>
...
async function apiUpdateEntry(id,p){
  const r=await fetch(`/api/entries/${id}`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(p)});
  return await r.json();
}
...
document.getElementById('saveNowBtn').onclick=async()=>{
  ...
  await apiAddEntry({day: dayIso, meal: state.meal, ts: ts.getTime(),
    note: document.getElementById('note').value||'',
    calories: clampInt(document.getElementById('calories').value,0),
  });
  document.getElementById('note').value='';
